...
;;;; YKK-specific code below

(define (mtable-copy mtable)
  (call-with-values make-cell-for-generic
    (lambda (generic get-perform set-perform!)
      (really-make-method-table
       (method-table-methods mtable)
       (method-table-prototype mtable)
       generic
       get-perform
       set-perform!
       (method-table-id mtable)))))

(define-syntax with-generic
  (syntax-rules ()
    ((_ ?name ?mtable expr ...)
     (let* ((?mtable (mtable-copy ?mtable))
            (?name (make-generic ?mtable)))
       expr ...))))


;; The following code redefines define-method so that it
;; returns a new method table instead of destructively
;; modifying it.
;; It is *not* currently being used.  It could be used
;; to create a more complete functionally updating generic system.
(define-syntax ykk-define-method
  (syntax-rules ()
    ((define-method ?mtable ?formals ?body ...)
     (add-method ?mtable
                 (method-info ?mtable ?formals ?body ...)))))

(define-generic add-method &add-method (method info))

(define (add-to-method-table mtable info)
  (let* ((lst (insert-method info (method-table-methods mtable)))
         (new-mtable (mtable-copy mtable)))
    ((method-table-set-perform! new-mtable)
     (methods->perform lst (method-table-prototype mtable)))
    new-mtable))
...
(define-syntax assert-string
  (syntax-rules (=>)
      ((_ body => string)
       (assert
        (let-string-output-port body) => string))))
        
(define-generic show &show)

(define-method &show (foo)
  (display foo))

(assert-string (show "foo bar") => "foo bar")

(define-method &show ((foo :number))
  (display "number: ")
  (display foo))

(define-method &show ((foo :string) "next" next)
  (display "string: ")
  (next))

(assert-string (show 5) => "number: 5")
(assert-string (show "foo bar") => "string: foo bar")

(with-generic show &show
  (define-method &show ((foo :string))
    (display "shadowed string: ")
    (next-method))

  (assert-string (show "fiz bang") => "shadowed string: fiz bang"))

(assert-string (show "baz gob") => "string: baz gob")
